...
<script type="text/javascript">
	...
	function formatAction(val, row) {
		return "<a href='javascript:openFinishTaskTab("
				+ row.id
				+ ")'>办理任务</a>&nbsp;<a target='_blank' href='${pageContext.request.contextPath}/task/showCurrentView.do?taskId="
				+ row.id + "'>查看当前流程图</a>"
	}

	function openFinishTaskTab(taskId) {
		$.post("${pageContext.request.contextPath}/task/redirectPage.do", {
			taskId : taskId
		}, function(result) {
			window.parent.openTab('办理任务', result.url + "?taskId=" + taskId,
					'icon-check');
		}, "json");
	}
</script>
...
<body style="margin: 1px">
	<table id="dg" title="待办任务管理" class="easyui-datagrid" fitColumns="true"
		...
		url="${pageContext.request.contextPath}/task/list.do?userId=${currentMemberShip.user.id }"
		fit="true" toolbar="#tb">
		<thead>
			<tr>
